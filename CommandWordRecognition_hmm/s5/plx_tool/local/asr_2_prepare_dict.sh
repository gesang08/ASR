...
cat $text_dir/word.list_by_aishell1_new |grep -v '<FIL/>' |grep -v '<SPK/>' |grep -v '<STA/>' |grep -v '<NON/>' |grep -v '<NPS/>' |grep -v '\*\*' > $dict_dir/words.txt ||exit 1;
echo `wc -l $text_dir/word.list_by_aishell1_new`
echo `wc -l $dict_dir/words.txt`
...
awk 'NR==FNR{words[$1]; next;} ($1 in words)' \
  $dict_dir/lexicon-en/words-en.txt $dict_dir/lexicon-en/lexicon-en_v1.txt |\
  egrep -v '<.?s>' > $dict_dir/lexicon-en/lexicon_iv_fromv1.txt || exit 1;

awk 'NR==FNR{words[$1]; next;} !($1 in words)' \
  $dict_dir/lexicon-en/lexicon-en_v1.txt $dict_dir/lexicon-en/words-en.txt |\
  egrep -v '<.?s>' > $dict_dir/lexicon-en/words-en_oov.txt || exit 1;
com

<<com
##not used in v2.0 lexicon prepare!!!
#####from panlixin split lexicon-en into three part!
echo "panlixin english lexicon!"
cat $dict_dir/lexicon-en/words-en.txt |grep '^[A-Z]\+$' >$dict_dir/lexicon-en/DAXIE_words-en.txt
cat $dict_dir/lexicon-en/words-en.txt |grep -v '^[A-Z]\+$' >$dict_dir/lexicon-en/danci_words-en.txt
cat $dict_dir/lexicon-en/danci_words-en.txt |grep -v '[^a-zA-Z]' >$dict_dir/lexicon-en/danci-en-words.txt
#$dict_dir/lexicon-en/lexicon-en-ch.txt this is create from panlixin
#$dict_dir/lexicon-en/en-char-dict.txt this is create from panlixin

local/create_oov_char_lexicon_for_eng.pl $dict_dir/lexicon-en/en-char-dict_new.txt \
  $dict_dir/lexicon-en/DAXIE_words-en.txt > $dict_dir/lexicon-en/lexicon-en-DAXIE.txt 

awk 'NR==FNR{words[$1]; next;} ($1 in words)' \
  $dict_dir/lexicon-en/danci-en-words.txt $dict_dir/lexicon-en/6181lexicon_xiaoxie |\
  egrep -v '<.?s>' > $dict_dir/lexicon-en/danci-en-iv1.txt || exit 1;

awk 'NR==FNR{words[$1]; next;} ($1 in words)' \
  $dict_dir/lexicon-en/danci-en-words.txt $dict_dir/lexicon-en/6181lexicon_DAxiaoxie |\
  egrep -v '<.?s>' > $dict_dir/lexicon-en/danci-en-iv2.txt || exit 1;


awk 'NR==FNR{words[$1]; next;} ($1 in words)' \
  $dict_dir/lexicon-en/danci-en-words.txt $dict_dir/lexicon-en/7777lexicon_xiaoxie |\
  egrep -v '<.?s>' > $dict_dir/lexicon-en/danci-en-iv1_from7777.txt || exit 1;

awk 'NR==FNR{words[$1]; next;} ($1 in words)' \
  $dict_dir/lexicon-en/danci-en-words.txt $dict_dir/lexicon-en/7777lexicon_DAxiaoxie |\
  egrep -v '<.?s>' > $dict_dir/lexicon-en/danci-en-iv2_from7777.txt || exit 1;
...
# give up all oov,change iv to phn
#cp $dict_dir/lexicon-en/lexicon-en-iv.txt $dict_dir/lexicon-en/lexicon-en-phn.txt 
...
if [ ! -f $dict_dir/cedict1/cedict_1_0_ts_utf-8_mdbg.txt ]; then
  echo "------------- Downloading cedit dictionary ---------------"
  mkdir -p $dict_dir/cedict1
  wget -P $dict_dir/cedict1 http://www.mdbg.net/chindict/export/cedict/cedict_1_0_ts_utf-8_mdbg.txt.gz
  gunzip $dict_dir/cedict1/cedict_1_0_ts_utf-8_mdbg.txt.gz
fi

cat $dict_dir/cedict1/cedict_1_0_ts_utf-8_mdbg.txt | grep -v '#' | awk -F '/' '{print $1}' |\
 perl -e '
  while (<STDIN>) {
    @A = split(" ", $_);
    print $A[1];
 ...
 ' | sort -k1 > $dict_dir/cedict1/ch-dict.txt || exit 1;
...
cat $dict_dir/aishell2_lexicon.txt > $dict_dir/cedict/ch-dict.txt || exit 1;
...
cat $dict_dir/051_lexicon_6 |\
  ...
  print $out_line;
  ' >  $dict_dir/lexicon-ch/ch-char-dict-mp_051_new.txt || exit 1;

# get lexicon for Chinese OOV words
local/create_oov_char_lexicon.pl $dict_dir/lexicon-ch/ch-char-dict-mp_051_new.txt \
  $dict_dir/lexicon-ch/words-ch-oov.txt > $dict_dir/lexicon-ch/lexicon-ch-oov_051_new.txt || exit 1;

# seperate multiple prons for Chinese OOV lexicon
cat $dict_dir/lexicon-ch/lexicon-ch-oov_051_new.txt |\
  perl -e '
  my @entry;
  my @entry1;
  while (<STDIN>) {
  ...
  ' > $dict_dir/lexicon-ch/lexicon-ch-oov-mp_051_new.txt || exit 1;
com

#stage 4
<<com
# compose IV and OOV lexicons for Chinese
cat $dict_dir/lexicon-ch/lexicon-ch-iv.txt |awk '{for(i=2;i<=NF;i=i+2) printf("%s%s ",$i,$(i+1)); printf"\n"}' \
>$dict_dir/lexicon-ch/lexicon-ch-iv_pronunce.txt
cat  $dict_dir/lexicon-ch/lexicon-ch-iv.txt |awk '{print $1}' > $dict_dir/lexicon-ch/lexicon-ch-iv_word.txt
paste -d ' ' $dict_dir/lexicon-ch/lexicon-ch-iv_word.txt $dict_dir/lexicon-ch/lexicon-ch-iv_pronunce.txt |\
sed 's/$0//g' |sed 's/_//g' >$dict_dir/lexicon-ch/lexicon-ch-iv_new.txt
cat  $dict_dir/lexicon-ch/lexicon-ch-oov-mp_051_new.txt |sed 's/5/0/g' >$dict_dir/lexicon-ch/lexicon-ch-oov-mp_051_new_new.txt
cat $dict_dir/lexicon-ch/lexicon-ch-oov-mp_051_new_new.txt $dict_dir/lexicon-ch/lexicon-ch-iv_new.txt |\
  awk '{if (NF > 1 && $2 ~ /[A-Za-z0-9]+/) print $0;}' > $dict_dir/lexicon-ch/lexicon-ch.txt || exit 1;

#a-z convert A-Z
cat $dict_dir/lexicon-ch/lexicon-ch.txt |tr a-z A-Z > $dict_dir/lexicon-ch/lexicon-ch_new.txt
com
<<com
# stdge 5
# convert Chinese pinyin to CMU format
cat $dict_dir/lexicon-ch/lexicon-ch_new.txt | sed -e 's/U:/V/g' | sed -e 's/ R\([0-9]\)/ ER\1/g'|\
  utils/pinyin_map.pl conf/pinyin2cmu > $dict_dir/lexicon-ch/lexicon-ch-cmu.txt || exit 1;
# combine English and Chinese lexicons
cat $dict_dir/lexicon-en/lexicon_iv_fromv1.txt $dict_dir/lexicon-ch/lexicon-ch-cmu.txt |\
...
# Add to the lexicon the silences, noises etc.
 (echo '!SIL SIL'; echo '<FIL/> FIL'; echo '<SPK/> SPK'; echo '<STA/> STA'; echo '<NON/> NON'; echo '<NPS/> NPS'; echo '** **'; echo '<UNK> SPN') | 
 cat - $dict_dir/lexicon1.txt  > $dict_dir/lexicon.txt || exit 1;

echo "$0: ASRproject dict preparation succeeded"
exit 0;
com
