...
# alignments for it.
#local/nnet3/run_ivector_common_teacherwang.sh --stage 3 || exit 1;
...
if [ $stage -le 12 ]; then
  ...
  utils/mkgraph.sh --self-loop-scale 1.0 data/lang_test_teacherwang_lm6_20  $dir $dir/graph_lm6_20
 # utils/mkgraph.sh --self-loop-scale 1.0 data/lang_test_teacherwang_lm5_10  $dir $dir/graph_lm5_10
fi

if [ $stage -le 13 ]; then
  graph_dir=$dir/graph_lm5_10
  for test_set in  test_teacher_wang; do
    steps/nnet3/decode.sh --acwt 1.0 --post-decode-acwt 10.0 \
      --nj 1 --cmd "$decode_cmd" \
      --online-ivector-dir exp/nnet3/ivectors_$test_set \
      $graph_dir data/${test_set}_hires_online $dir/decode_${test_set}_lm5_10 || exit 1;
  done
fi
...
if [ $stage -le 15 ]; then
  graph_dir=$dir/graph_lm5
  for test_set in  test_teacher_wang; do
    steps/nnet3/decode.sh --acwt 1.0 --post-decode-acwt 10.0 \
      --nj 1 --cmd "$decode_cmd" \
      --online-ivector-dir exp/nnet3/ivectors_$test_set \
      $graph_dir data/${test_set}_hires_online $dir/decode_${test_set}_lm5 || exit 1;
  done
fi
if [ $stage -le 16 ]; then
  graph_dir=$dir/graph_lm6_20
  for test_set in  test_teacher_wang; do
    steps/nnet3/decode.sh --acwt 1.0 --post-decode-acwt 10.0 \
      --nj 1 --cmd "$decode_cmd" \
      --online-ivector-dir exp/nnet3/ivectors_$test_set \
      $graph_dir data/${test_set}_hires_online $dir/decode_${test_set}_lm6_20 || exit 1;
  done
fi
