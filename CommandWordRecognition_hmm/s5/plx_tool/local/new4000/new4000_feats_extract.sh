...
if [ $stage -le 1 ]; then
  ...
  steps/make_mfcc.sh --cmd "$train_cmd" --nj 70 data/${train_set}_sp \
...
if [ $stage -le 3 ]; then
  ...
  for datadir in ${train_set}_sp ; do
    steps/make_mfcc_pitch$online_affix.sh --nj 70 --mfcc-config conf/mfcc_hires.conf \
...
echo "feature extraction is finished~"
<<com
if [ $stage -le 4 ]; then
