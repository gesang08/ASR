...
#text_dir=data
dict_dir=panlixin_test/aishelldict
<<com
mkdir -p $dict_dir
...
cat $text_dir/word.list_new |grep -v '<FIL/>' |grep -v '<SPK/>' |grep -v '<STA/>' |grep -v '<NON/>' |grep -v '<NPS/>' |grep -v '\*\*' > $dict_dir/words.txt ||exit 1;
cp panlixin_test/word_aishell1 $dict_dir/words.txt
...
cat $dict_dir/cedict/ch-dict.txt |\
  ...
  while (<STDIN>) {
    ...
    $word_len = length($A[0]);
    $proun_len = @A - 1 ;
    if (2*$word_len == $proun_len) {print $_;}
  }
...
cat $dict_dir/cedict/ch-dict-1.txt |\
  awk '{for(i=2; i<=NF; i=i+2) printf("%s%s\n",$i,$(i+1))}' |\
...
cat $dict_dir/051_lexicon_plx_new_remove_multi_pronunce |\
  perl -e '
  my $prev = "";
  my $out_line = "";
  while (<STDIN>) {
  ...
  print $out_line;
  ' >  $dict_dir/lexicon-ch/ch-char-dict-mp_051_plx_new.txt || exit 1;

# get lexicon for Chinese OOV words
local/create_oov_char_lexicon.pl $dict_dir/lexicon-ch/ch-char-dict-mp_051_plx_new.txt \
  $dict_dir/lexicon-ch/words-ch-oov.txt > $dict_dir/lexicon-ch/lexicon-ch-oov_051_plx_new.txt || exit 1;

# seperate multiple prons for Chinese OOV lexicon
cat $dict_dir/lexicon-ch/lexicon-ch-oov_051_plx_new.txt |\
  perl -e '
  my @entry;
  my @entry1;
  while (<STDIN>) {
  ...
  ' > $dict_dir/lexicon-ch/lexicon-ch-oov-mp_051_plx_new.txt || exit 1;
<<com
# compose IV and OOV lexicons for Chinese
cat $dict_dir/lexicon-ch/lexicon-ch-oov-mp_051.txt $dict_dir/lexicon-ch/lexicon-ch-iv_051_new.txt |\
...
cat $dict_dir/lexicon-en/lexicon-en_latest.txt $dict_dir/lexicon-ch/lexicon-ch-cmu.txt |\
