...
cat $dict_dir/051_new_lexicon |\
  ...
  print $out_line;
  ' >  $dict_dir/lexicon-ch/ch-char-dict-mp_051.txt || exit 1;

# get lexicon for Chinese OOV words
local/create_oov_char_lexicon.pl $dict_dir/lexicon-ch/ch-char-dict-mp_051.txt \
  $dict_dir/lexicon-ch/words-ch-oov.txt > $dict_dir/lexicon-ch/lexicon-ch-oov_051.txt || exit 1;

# seperate multiple prons for Chinese OOV lexicon
cat $dict_dir/lexicon-ch/lexicon-ch-oov_051.txt |\
  perl -e '
  my @entry;
  my @entry1;
  while (<STDIN>) {
  ...
  ' > $dict_dir/lexicon-ch/lexicon-ch-oov-mp_051.txt || exit 1;
